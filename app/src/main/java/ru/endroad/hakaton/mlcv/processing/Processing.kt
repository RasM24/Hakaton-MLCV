...
object Processing {
	...
	private val trainNumberRegex = Regex.fromLiteral("([0-9]{4})")
	...
	fun processText(data: List<String>): String =
		data.filter { it.matches(trainNumberRegex) || it.matches(autoNumber) }
			.joinToString(prefix = "Номера транспортных средств: ")

	fun processLabels(data: List<String>): String = data[0]

	fun processLuminosity(data: Double): AlertLuminosityStatus =
		when {
			data < 10.0 -> AlertLuminosityStatus.DARK
			data > 120.0 -> AlertLuminosityStatus.LIGHT
